...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] == 'add') {
        ...
        try {
            ...
            $success = "Toko/Pelanggan berhasil ditambahkan!";
        } catch (PDOException $e) {
            $error = "Gagal menambah toko: " . $e->getMessage();
        }
    }
}

$stmt = $pdo->query("SELECT * FROM tb_toko ORDER BY id_toko DESC");
$tokos = $stmt->fetchAll();
...
<div class="flex-1 overflow-y-auto p-8 animate-fade-in">
    <header class="flex justify-between items-center mb-10">
        <div>
            <h2 class="text-3xl font-extrabold text-navy-900 tracking-tight">Master Toko / Cabang</h2>
            <p class="text-slate-500 font-medium mt-1">Manajemen data pelanggan & toko peminta barang.</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] border-b border-slate-50">
                        ...
                        <th class="px-8 py-6">Nama Toko</th>
                ...
                <tbody class="divide-y divide-slate-50 text-sm">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors group">
                        <td class="px-8 py-6">
                            <span class="font-black text-navy-900 group-hover:text-blue-600 transition-colors"><?= htmlspecialchars($t['kode_toko']) ?></span>
                        </td>
                        <td class="px-8 py-6">
                            <p class="font-bold text-slate-700"><?= htmlspecialchars($t['nama_toko']) ?></p>
                        </td>
                        <td class="px-8 py-6 text-slate-500 font-medium">
                            <?= htmlspecialchars($t['kontak']) ?>
                        </td>
...
<div id="modalAdd" class="fixed inset-0 bg-navy-900/60 backdrop-blur-sm flex items-center justify-center hidden z-50">
    <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg mx-4 overflow-hidden animate-fade-in">
        <div class="px-10 py-8 border-b border-slate-50 bg-slate-50/50 flex justify-between items-center">
            <div>
                <h3 class="font-black text-2xl text-navy-900 tracking-tight">Toko Baru</h3>
                <p class="text-xs text-slate-500 font-medium">Registrasi toko/cabang ke sistem.</p>
            </div>
        ...
        <form method="POST" action="master_toko.php" class="p-10">
            <input type="hidden" name="action" value="add">
            <div class="space-y-6">
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Kode Toko</label>
                    <input type="text" name="kode_toko" required placeholder="TK-001" class="w-full bg-slate-50 border-none rounded-2xl p-4 text-sm font-bold text-navy-900 focus:ring-2 focus:ring-amber-500 transition-all outline-none">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Nama Toko</label>
                    <input type="text" name="nama_toko" required placeholder="Toko..." class="w-full bg-slate-50 border-none rounded-2xl p-4 text-sm font-bold text-navy-900 focus:ring-2 focus:ring-amber-500 transition-all outline-none">
                </div>
                <div>
            ...
            <div class="flex gap-4 mt-10">
                ...
                <button type="submit" class="flex-1 py-4 rounded-2xl bg-navy-900 text-white font-bold text-sm shadow-xl shadow-navy-900/20 hover:bg-navy-800 transition-all">Simpan Toko</button>
            </div>
        </form>
    </div>
</div>
